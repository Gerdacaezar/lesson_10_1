def filter_by_state(list_of_id: list[dict], needs_state: str = 'EXECUTED') -> list[dict] | None:
    """Функция фильтрации списка ID по 'state' (по умолчанию 'EXECUTED')"""
    result = []
...
def sort_by_date(list_of_id: list[dict], sort_range: str = 'True') -> list[dict] | None:
    """Функция сортировки списка ID по 'date' (по умолчанию (sort_range = True) по убыванию)"""
    return sorted(list_of_id, key=lambda x: x['date'], reverse=True if sort_range == 'True' else False)
